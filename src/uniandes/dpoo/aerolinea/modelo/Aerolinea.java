...
public class Aerolinea
{
    ...
    {
        Iterator<Vuelo> flightList = this.getVuelos().iterator();
        while ( flightList.hasNext() ) {
        	Vuelo flight = flightList.next();
        	if (flight.getRuta().getCodigoRuta() == codigoRuta) {
        		if (flight.getFecha() == fechaVuelo) {
        			return flight;
        		}
        	}
        }
    ...
    {
    	Collection<Tiquete> ticketCollection = new ArrayList<Tiquete>();
    	Iterator<Vuelo> flightList = this.getVuelos().iterator();
        while ( flightList.hasNext() ) {
        	Vuelo flight = flightList.next();
        	Iterator<Tiquete> ticketList = flight.getTiquetes().iterator();
        	while (ticketList.hasNext()) {
        		ticketCollection.add(ticketList.next());
        	}
        		}
        return null;
